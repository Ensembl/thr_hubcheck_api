...
def hub_check(hub_url):
    ...
    if not Path(hubcheck).exists():
        ...
        if platform == "linux":
            subprocess.run(
                ["curl", "-o", "hubCheck", "http://hgdownload.soe.ucsc.edu/admin/exe/linux.x86_64/hubCheck"]
            )
        elif platform == "darwin":
            subprocess.run(
                ["curl", "-o", "hubCheck", "http://hgdownload.soe.ucsc.edu/admin/exe/macOSX.x86_64/hubCheck"]
            )
